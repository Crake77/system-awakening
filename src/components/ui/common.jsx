...
export function Bar({ value, max, color, label, h = 14 }) {
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: h, background: "#18183a", borderRadius: 3, overflow: "hidden", border: "1px solid #323254" }}>
...
export function Btn({ onClick, disabled, children, color = "#0af", small, glow }) {
  return (
    <button onClick={onClick} disabled={disabled} style={{
      ...
      border: "1px solid " + (disabled ? "#3a3a58" : hexToRgba(color, 0.55)),
...
export function TabBtn({ active, onClick, children, alert }) {
  return (
    <button onClick={onClick} style={{
      ...
      color: active ? "#0ff" : "#aabbdd",
      border: active ? "1px solid #0af55" : "1px solid #2a2a4a",
...
export function Sec({ title, children, color = "#0af", border }) {
  return (
    <div style={{ background: "#1a1a3a", borderRadius: 6, padding: 9, border: `1px solid ${border || "#2a2a4a"}`, marginBottom: 7 }}>
      {title ? (
...
export function MatCost({ mats, state }) {
  return (
    <span style={{ fontSize: 8, color: "#aabbdd" }}>
...
export function StatBox({ label, value, color }) {
  return (
    <div style={{ background: "#18183a", borderRadius: 3, padding: 3, textAlign: "center", border: "1px solid #282850" }}>
      <div style={{ fontSize: 7, color: "#99aacc" }}>{label}</div>
